...
export class MemberContactsService {
  ...
  async findOneByMemberContact(memberContact: string): Promise<DetailApiResponseFormat> {
    ...
    if (detail) {
      ...
      try {
        ...
        if (detail) {
          this.logger.log('✅ Tìm thấy dữ liệu sau khi scrape');
        } else {
          this.logger.error('❌ Vẫn không tìm thấy dữ liệu sau khi scrape');
        }
      } catch (error) {
        this.logger.error(`❌ Lỗi khi gọi API scrape: ${error.message}`);
        throw new NotFoundException(`Không tìm thấy member contact với mã: ${memberContact} và không thể scrape dữ liệu`);
      }
    }
    this.logger.log('========================================');
  ...
  private async callScrapeApi(memberContact: string): Promise<void> {
    const scrapeUrl = 'http://14.225.0.186:5000/api/scrape/member-contact-detail';
    const requestBody = {
    ...
    this.logger.log('========================================');
    this.logger.log('🔄 [SCRAPE] Gọi API scrape member contact detail');
    this.logger.log('========================================');
